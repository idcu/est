...
本目录包含 EST 框架的部署相关配置文件，支持 Docker、Docker Compose、Kubernetes 和 Serverless 部署方式。
...
EST 框架支持多种部署方式，你可以根据需求选择：
| 部署方式 | 适用场景 | 优势 |
|---------|---------|------|
| **Docker** | 单机部署、开发测试 | 简单快速、环境一致 |
| **Docker Compose** | 多容器应用、本地开发 | 一键启动多服务 |
| **Kubernetes** | 生产环境、高可用、微服务 | 自动伸缩、自我修复、服务发现 |
| **Serverless** | 事件驱动、按需付费、低流量 | 无需管理服务器、按调用付费 |
...
```bash
cd deploy/scripts
build-docker.bat 2.3.0-SNAPSHOT
```
...
./build-docker.sh 2.3.0-SNAPSHOT
```
...
```bash
cd deploy/docker
docker build -t est-demo:2.3.0-SNAPSHOT -f Dockerfile ../..
```
...
| APP_VERSION | 2.3.0-SNAPSHOT | 应用版本 |
| LOG_LEVEL | INFO | 日志级别 |
...
#### 更新镜像
```bash
kubectl set image deployment/est-demo est-demo=est-demo:2.3.0-SNAPSHOT -n est
```
...
Serverless 是一种按需付费的计算执行模式，你无需管理服务器，只需编写代码即可。

### 支持的云平台

- **AWS Lambda** - 亚马逊云函数服务
- **Azure Functions** - 微软 Azure 函数服务
- **阿里云函数计算** - 阿里云 Serverless 计算服务
- **Google Cloud Functions** - Google Cloud 函数服务

### 快速开始

详细的 Serverless 部署指南请参考：[Serverless 部署文档](./serverless/README.md)
...
public class HelloWorldFunction implements HttpServerlessFunction {
    ...
    public ServerlessResponse handle(ServerlessRequest request, Map<String, Object> context) {
        ...
        return ServerlessResponse.ok("{\"message\":\"Hello, " + name + "!\"}");
    }
}
```
