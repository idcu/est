...
public class DefaultSeq<T> implements Seq<T> {

    private final List<T> items;
    private final boolean mutable;

    private DefaultSeq(List<T> items, boolean mutable) {
        this.items = items;
        this.mutable = mutable;
    }

    public static <T> DefaultSeq<T> empty() {
        return new DefaultSeq<>(Collections.emptyList(), false);
    }

    @SafeVarargs
    public static <T> DefaultSeq<T> of(T... elements) {
        if (elements == null || elements.length == 0) {
            return empty();
        }
        return new DefaultSeq<>(new ArrayList<>(Arrays.asList(elements)), false);
    }

    public static <T> DefaultSeq<T> from(Iterable<T> iterable) {
        if (iterable == null) {
            return empty();
        }
        if (iterable instanceof DefaultSeq) {
            return (DefaultSeq<T>) iterable;
        }
        List<T> list = new ArrayList<>();
        for (T item : iterable) {
            list.add(item);
        }
        return new DefaultSeq<>(list, false);
    }

    public static <T> DefaultSeq<T> from(Stream<T> stream) {
        if (stream == null) {
            return empty();
        }
        return new DefaultSeq<>(stream.collect(Collectors.toList()), false);
    }

    private <R> DefaultSeq<R> newSeq(List<R> items) {
        return new DefaultSeq<>(items, false);
    }
