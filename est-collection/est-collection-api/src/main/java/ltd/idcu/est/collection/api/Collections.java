...
public final class Collections {
    ...
    public interface CollectionProvider {
        ...
        <T> LazyCollection<T> lazyFromIterable(Iterable<T> iterable);
        ...
        <T> MutableCollection<T> mutableFromIterable(Iterable<T> iterable);
        <T> MutableCollection<T> mutableOf(T... elements);
        <T> Collection<T> immutableFromIterable(Iterable<T> iterable);
        <T> Collection<T> immutableOf(T... elements);
        IntCollection intOf(int... elements);
        IntCollection intFromArray(int[] array);
        IntCollection intRange(int start, int end, int step);
    }
    ...
    @SuppressWarnings("unchecked")
    public static <T> LazyCollection<T> lazyOf(T... elements) {
        if (elements == null || elements.length == 0) {
            return (LazyCollection<T>) LazyEmptyCollection.INSTANCE;
        }
        return lazyFromIterable(Arrays.asList(elements));
    }

    public static <T> LazyCollection<T> lazyFromIterable(Iterable<T> iterable) {
        return requireProvider().lazyFromIterable(iterable);
    }

    public static <T> LazyCollection<T> lazyFrom(Iterable<T> iterable) {
        return lazyFromIterable(iterable);
    }

    @SuppressWarnings("unchecked")
    public static <T> MutableCollection<T> mutableOf(T... elements) {
        if (elements == null || elements.length == 0) {
            return requireProvider().mutableOf();
        }
        return requireProvider().mutableOf(elements);
    }

    public static <T> MutableCollection<T> mutableFromIterable(Iterable<T> iterable) {
        return requireProvider().mutableFromIterable(iterable);
    }

    @SuppressWarnings("unchecked")
    public static <T> Collection<T> immutableOf(T... elements) {
        if (elements == null || elements.length == 0) {
            return empty();
        }
        return requireProvider().immutableOf(elements);
    }

    public static <T> Collection<T> immutableFromIterable(Iterable<T> iterable) {
        return requireProvider().immutableFromIterable(iterable);
    }

    public static IntCollection intOf(int... elements) {
        return requireProvider().intOf(elements);
    }

    public static IntCollection intFromArray(int[] array) {
        return requireProvider().intFromArray(array);
    }

    public static IntCollection intRange(int start, int end) {
        return intRange(start, end, 1);
    }

    public static IntCollection intRange(int start, int end, int step) {
        return requireProvider().intRange(start, end, step);
    }
    ...
    private static final class EmptyCollection<T> implements Collection<T> {
        ...
        @Override
        public <K1, K2> java.util.Map<K1, java.util.Map<K2, Collection<T>>> groupByMultiple(
                java.util.function.Function<? super T, ? extends K1> keySelector1,
                java.util.function.Function<? super T, ? extends K2> keySelector2) {
            return new java.util.HashMap<>();
        }

        @Override
        public <K, A, R> java.util.Map<K, R> aggregateBy(
                java.util.function.Function<? super T, ? extends K> keySelector,
                java.util.function.Supplier<A> initialSupplier,
        ...
        @Override
        public Pair<Collection<T>, Collection<T>> partition(java.util.function.Predicate<? super T> predicate) {
        ...
        @Override
        public Collection<T> shuffle(java.util.Random random) {
            return empty();
        }
    ...
    private static final class LazyEmptyCollection<T> implements LazyCollection<T> {

        @SuppressWarnings("rawtypes")
        private static final LazyEmptyCollection INSTANCE = new LazyEmptyCollection();
        ...
        @Override
        public Collection<T> plus(T element) {
            return LazyCollection.of(element);
        }

        @Override
        public Collection<T> plusAll(Iterable<? extends T> elements) {
            return LazyCollection.from((Iterable<T>) elements);
        }
        ...
        @Override
        public Collection<T> union(Iterable<? extends T> other) {
            return LazyCollection.from((Iterable<T>) other);
        }
        ...
        @SuppressWarnings("unchecked")
        public Collection<java.util.List<T>> chunked(int size) {
            return (Collection<java.util.List<T>>) this;
        }
        ...
        @SuppressWarnings("unchecked")
        public <U> Collection<Pair<T, U>> zip(Collection<U> other) {
            return (Collection<Pair<T, U>>) this;
        }

        @Override
        @SuppressWarnings("unchecked")
        public Collection<Pair<Integer, T>> zipWithIndex() {
            return (Collection<Pair<Integer, T>>) this;
        }
