...
EST Core 就像是一个"智能管家"。想象一下你经营一家公司，有很多不同的部门：市场部、技术部、财务部... 每个部门都需要用到其他部门的工作。

**传统方式**：每个部门需要什么都自己去创建，导致重复工作，关系混乱。

**EST Core 方式**：你把所有部门都告诉管家，管家会负责：
- 记住每个部门的位置和职责
- 当一个部门需要另一个部门帮忙时，管家会自动安排
- 确保每个人拿到的都是正确的、需要的东西

### 核心特点

- 🎯 **简单易用** - 几行代码就能创建和管理组件
- ⚡ **高性能** - 三层 Map 实现，快速查找组件
- 🔒 **类型安全** - 编译时就能发现类型错误
- 🎨 **灵活强大** - 支持多种注入方式、作用域、生命周期管理
...
EST Core 和 est-collection 可以完美配合使用！
...
1. 重构代码，消除循环依赖
2. 使用懒加载
3. 使用 setter 注入替代构造函数注入

### Q: 性能如何？

A: EST Core 使用三层 Map 实现，性能非常好，完全可以满足生产环境需求。
...
- 查看 [est-features-data](../est-features/est-features-data/) 进行数据持久化
