# Fix compatibility modules - remove escape characters issue
...
foreach ($mapping in $moduleMappings) {
    ...
    Write-Host "Fixing: $oldName -> $newName"
    
    # Fix compatibility pom.xml for the main module
...
"@
    ...
    Set-Content -Path $oldPomPath -Value $compatPomContent -Encoding UTF8
    Write-Host "  Fixed: $oldPomPath"
...
"@
                    ...
                    Set-Content -Path $oldSubPomPath -Value $oldSubPomContent -Encoding UTF8
                    Write-Host "  Fixed: $oldSubPomPath"
                }
            }
        }
    }
    ...
    if ($subModules.Count -gt 0) {
        ...
        Write-Host "  Fixed modules in: $oldPomPath"
    }
}

Write-Host "`n✓ Compatibility modules fixed successfully!" -ForegroundColor Green
