package ltd.idcu.est.features.messaging.websocket;
...
public class WebSocketMessageConsumer implements MessageConsumer {
    ...
    public WebSocketMessageConsumer(WebSocketConnection connection, MessagingConfig config) {
    ...
    public Message receive(long timeout) {
        ...
        try {
            WebSocketConnection.WebSocketMessage webSocketMessage = connection.receive();
            if (webSocketMessage != null) {
                return convertMessage(webSocketMessage);
            }
        } catch (Exception e) {
            // Ignore
        }
        ...
        executor.submit(() -> {
            while (running.get() && !closed && connection.isConnected()) {
                try {
                    WebSocketConnection.WebSocketMessage webSocketMessage = connection.receive();
                    if (webSocketMessage != null) {
                        Message message = convertMessage(webSocketMessage);
                        Consumer<Message> topicHandler = handlers.get(webSocketMessage.getTopic());
    ...
    public void acknowledge(Message message) {
        // WebSocket doesn't need explicit acknowledgment
    }
    ...
    private Message convertMessage(WebSocketConnection.WebSocketMessage webSocketMessage) {
        String body = webSocketMessage.getBody() != null ? new String(webSocketMessage.getBody(), StandardCharsets.UTF_8) : null;
        
        return DefaultMessage.builder()
                .id(String.valueOf(System.currentTimeMillis()))
                .topic(webSocketMessage.getTopic())
                .body(body)
