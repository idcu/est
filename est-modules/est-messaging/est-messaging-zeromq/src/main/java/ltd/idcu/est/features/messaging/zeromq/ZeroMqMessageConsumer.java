...
public class ZeroMqMessageConsumer implements MessageConsumer {
    ...
    public ZeroMqMessageConsumer(ZeroMqConnection connection, MessagingConfig config) {
    ...
    public Message receive(long timeout) {
        ...
        try {
            ZeroMqConnection.ZeroMqMessage zmqMessage = connection.receive();
            if (zmqMessage != null) {
                return convertMessage(zmqMessage);
            }
        } catch (Exception e) {
            // Ignore
        }
        ...
        executor.submit(() -> {
            while (running.get() && !closed && connection.isConnected()) {
                try {
                    ZeroMqConnection.ZeroMqMessage zmqMessage = connection.receive();
                    if (zmqMessage != null) {
                        Message message = convertMessage(zmqMessage);
                        Consumer<Message> topicHandler = handlers.get(zmqMessage.getTopic());
    ...
    public void acknowledge(Message message) {
        // ZeroMQ doesn't need explicit acknowledgment
    }
    ...
    private Message convertMessage(ZeroMqConnection.ZeroMqMessage zmqMessage) {
        String body = zmqMessage.getBody() != null ? new String(zmqMessage.getBody(), StandardCharsets.UTF_8) : null;
        
        return DefaultMessage.builder()
                .id(String.valueOf(System.currentTimeMillis()))
                .topic(zmqMessage.getTopic())
                .body(body)
