# EST Web Router Web 路由模块 - 小白从入门到精�?
## 目录
...
## 什么是 EST Web Router�?### 用大白话理解

EST Web Router 就像是一�?**交通指挥中�?*。想象一下你的网站有很多页面，用户访问不同的 URL 需要找到对应的处理程序�?
**传统方式**：你需要自己写一大堆 if-else 或�?switch 来判�?URL 要调用哪个函数，很麻烦！

**EST Web Router 方式**：给你一套智能的路由系统，支持：
- 📝 **路径参数** - `/users/{id}` 匹配 `/users/123`
- 🔍 **查询参数** - `?page=1&limit=10`
- 📁 **路由分组** - 把相关的路由组织在一�?- 🌲 **路由�?* - 高性能�?Trie 树实�?
### 核心特�?
- 🎯 **简单易�?* - 几行代码就能定义路由
- 📦 **高性能** - 基于 Trie 树的路由匹配
- 🎨 **灵活强大** - 支持路径参数、通配符、路由分�?- 📋 **RESTful 友好** - 支持 GET、POST、PUT、DELETE �?HTTP 方法
...
public class FirstRouterApp {
    public static void main(String[] args) {
        System.out.println("=== EST Web Router 第一个示�?===\n");
        ...
        System.out.println("访问 http://localhost:8080/hello/张三 查看个性化问�?);
    }
}
```
...
router.get("/posts/{postId}/comments/{commentId}", (req, res) -> {
    ...
    res.send("文章 " + postId + " 的评�?" + commentId);
});
```
...
router.get("/response", (req, res) -> {
    ...
    res.status(201).send("已创�?);
    res.status(404).send("未找�?);
...
// �?不推荐：零散的路�?router.get("/api/users", listUsers);
...
```java
// �?推荐：清晰的参数�?router.get("/users/{userId}/posts/{postId}", handler);
...
- [est-web-middleware README](../est-web-middleware/README.md) - 中间件文�?- [est-web-session README](../est-web-session/README.md) - 会话管理文档
