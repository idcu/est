# EST Microservices 微服务模�?- 小白从入门到精�?
## 目录
...
EST Microservices 就像是一�?微服务工具箱"。想象一下你有一个大型系统，拆分成多个小服务�?
**传统方式**：服务之间找不到对方，一个服务挂了整个系统都瘫了，不知道性能怎么�?.. 很麻烦！

**EST Microservices 方式**：给你一套完整的微服务工具，里面有：
- 📡 **服务发现** - 服务之间能自动找到对�?- 🛡�?**熔断�?* - 一个服务挂了不会影响其他服�?- 📊 **性能监控** - 实时监控服务的性能指标

### 核心特点

- 🎯 **简单易�?* - 几行代码就能启用微服务功�?- �?**高性能** - 基于高性能的服务发现和熔断实现
- 🔧 **灵活扩展** - 可以自定义服务发现策略和熔断规则
- 🎨 **功能完整** - 服务发现、熔断器、性能监控一应俱�?
---
...
public class FirstMicroserviceApp {
    public static void main(String[] args) {
        System.out.println("=== EST Microservices 第一个示�?===\n");
        ...
        System.out.println("服务已注�? user-service");
        ...
        System.out.println("熔断器调用结�? " + result);
...
System.out.println("找到一个实�? " + one.getHost() + ":" + one.getPort());

Seq<ServiceInstance> all = discovery.findAll("user-service");
System.out.println("找到 " + all.size() + " 个实�?);
...
ServiceRegistry registry = ServiceRegistry.create();
Heartbeat heartbeat = registry.createHeartbeat(instance, 30); // 30秒心�?heartbeat.start();
...
CircuitBreakerConfig config = CircuitBreakerConfig.builder()
    .failureThreshold(5) // 5次失败后打开熔断�?    .waitDuration(30000) // 30秒后尝试半开状�?    .successThreshold(3) // 3次成功后关闭熔断�?    .build();
...
try {
    ...
    System.out.println("熔断器已打开，使用降级方�?);
...
System.out.println("熔断器状�? " + state); // CLOSED, OPEN, HALF_OPEN
...
### 2. 合理设置熔断器参�?
```java
// �?推荐：根据实际情况调�?CircuitBreakerConfig config = CircuitBreakerConfig.builder()
    .failureThreshold(10)
    .waitDuration(60000)
    .successThreshold(5)
    .build();

// �?不推荐：太敏感或太迟�?CircuitBreakerConfig badConfig = CircuitBreakerConfig.builder()
...
```java
// �?推荐：监控关键操�?monitor.start("db.query");
monitor.start("api.call");
...
### 4. 服务健康检�?
```java
// �?推荐：实现健康检�?ServiceInstance.builder()
    .healthCheckUrl("http://localhost:8080/health")
    .build();

// �?不推荐：不提供健康检�?ServiceInstance.builder()
    .build();
```
...
est-microservices/
├── est-discovery/      # 服务发现
├── est-circuitbreaker/ # 熔断器模�?└── est-performance/    # 性能监控
```
...
- [est-circuitbreaker README](./est-circuitbreaker/README.md) - 熔断器详细文�?- [示例代码](../../est-examples/est-examples-microservices/) - 微服务示例代�?- [EST Web Group](../est-web-group/README.md) - Web 模块
