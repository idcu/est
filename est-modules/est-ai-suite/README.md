...
EST AI Suite 就像是一个「AI 工具包」。想象一下你要在应用中集�?AI 功能�?
**传统方式**：每一�?AI 提供商都要写不同的代码，还要自己处理 API 调用、错误重试、Token 计费... 很麻烦！

**EST AI Suite 方式**：给你一套统一�?AI 工具，里面有�?- 🎁 **LLM 核心抽象** - 统一的大语言模型接口
- 🌟 **多提供商支持** - 支持 OpenAI、智谱、通义千问、文心一言、豆包、Kimi、Ollama
- 🤖 **AI 助手** - 对话�?AI 助手
- 💻 **代码生成** - 自动生成代码

### 核心特�?
- �?**简单易�?* - 统一�?API，不用关心底层实�?- �?**高性能** - 优化�?API 调用，支持重试和缓存
- 🔧 **灵活扩展** - 可以自定�?LLM 提供�?- 🎯 **功能完整** - LLM、AI 助手、代码生成一应俱�?
---
...
public class FirstAiApp {
    public static void main(String[] args) {
        System.out.println("=== EST AI Suite 第一个示�?===\n");
...
ChatMessage userMessage = ChatMessage.user("用户的问�?);
ChatMessage assistantMessage = ChatMessage.assistant("AI 的回�?);
ChatMessage systemMessage = ChatMessage.system("你是一�?helpful 的助�?);
```
...
String response = assistant.chat("如何�?Java 写一�?Hello World?");
...
String response2 = conversation.send("我叫什么名�?"); // AI 会记住上下文
...
ChatRequest request = ChatRequest.builder()
    .addMessage(ChatMessage.user("北京今天天气怎么�?"))
    .addTool(weatherTool)
    .build();
...
```java
// �?推荐：使用环境变�?String apiKey = System.getenv("OPENAI_API_KEY");
...
LlmClient badClient = OpenAiLlmClient.builder()
    .apiKey("sk-this-is-a-secret") // 不要提交到代码仓�?    .build();
```
...
### 4. 提示词工�?
```java
// �?推荐：清晰的系统提示和用户提�?ChatRequest request = ChatRequest.builder()
...
// �?不推荐：模糊的提�?ChatRequest badRequest = ChatRequest.builder()
    .addMessage(ChatMessage.user("写代�?))
    .build();
```
...
├── est-llm/           # LLM 提供商实现（OpenAI、Zhipu、Qwen、Ernie、Doubao、Kimi、Ollama�?└── est-ai-assistant/  # AI 助手和代码生�?```
