...
### 用大白话理解
est-rbac 就像公司�?权限管理系统"。每个人（用户）有不同的职位（角色），每个职位能做不同的事（权限）。比�?员工"只能看自己的资料�?经理"能审批申请，"管理�?能管理所有人�?
### 核心特点
- **标准 RBAC 模型**：用�?角色-权限三层架构
- **权限继承**：支持角色层级和权限继承
- **动态权�?*：运行时动态调整权�?- **注解支持**：声明式权限控制
...
public class ProfileController {
    ...
    public void updateProfile(@PathParam Long id, @Body Profile profile) {
        ...
        if (!currentUser.getId().equals(id)) {
            throw new AccessDeniedException("只能修改自己的资�?);
        }
...
| 场景 | 推荐做法 | 说明 |
|------|---------|------|
| 权限设计 | 细粒度权�?+ 粗粒度角�?| 权限细分，角色组�?|
| 命名规范 | module:action:resource | user:create:order |
| 权限检�?| 优先用注解，复杂用代�?| 注解简洁，代码灵活 |
| 缓存策略 | 权限数据需要缓�?| 减少数据库查�?|
| 审计日志 | 记录所有权限变�?| 便于追溯和审�?|

### �?不推荐做�?
```java
// �?不要硬编码权限检�?if (user.getRole().equals("ADMIN")) {
    // 直接放行
}

// �?应该使用统一的权限服�?if (rbacService.hasPermission(user, "user:delete")) {
    // 通过权限检�?}
```
